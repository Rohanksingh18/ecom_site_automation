...
class ProductsAPIHelper:
    ...
    def call_list_products(self, payload=None):
        """
        If the max number of products per page is not provided,
        then use the max to reduce the number of calls.
        Starts from 2 because this will be used for page number and page 1 is fetched already.
        """
        if not payload:
