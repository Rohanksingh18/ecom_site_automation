...
# Verify 'POST /customers' creates user with email and password only (tcid20).
...
# Verify 'create customer' fails if email exists (tcid21).
@pytest.mark.customers
@pytest.mark.tcid21
def test_create_customer_fails_for_existing_email():
    ...
    # verify for correct error message
...
# Verify 'POST /customers' fails if password not provided (tcid22).
@pytest.mark.customers
@pytest.mark.tcid22
