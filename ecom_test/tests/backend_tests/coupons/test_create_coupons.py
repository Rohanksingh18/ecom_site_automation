...
def test_create_coupon_with_invalid_type():
    """
    Test case (tcid47) is to verify that using an invalid value for the 'discount_type' parameter during coupon creation
    will result in the correct error message.
    """
    # Create payload dictionary and call API.
    pyload = dict()
    pyload['code'] = generate_random_coupon_code(suffix='tcid47', length=5)
    # The code is given the suffix "tcid47" and has a length of 5.
    pyload['discount_type'] = generate_random_string()  # The payload is assigned a randomly generated string.
